...
import { defaultAbiCoder, SigningKey, Interface, arrayify, hexConcat } from "ethers/lib/utils"
...
describe("OffchainResolver", function () {
    ...
    describe("resolve()", async () => {
        it("returns a CCIP-read error", async () => {
            await expect(resolver.resolve(dnsName(name), "0x")).to.be.revertedWith("OffchainLookup")
        })
    })
    ...
    describe("resolveWithProof()", async () => {
        ...
        before(async () => {
            ...
            callData = resolver.interface.encodeFunctionData("resolve", [dnsName(name), addrData])
