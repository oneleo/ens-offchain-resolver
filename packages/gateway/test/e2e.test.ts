...
import { BaseProvider, BlockTag, TransactionRequest, Network } from "@ethersproject/providers"
...
describe("End to end test", () => {
    ...
    beforeAll(async () => {
        resolver = (
            await deploySolidity(OffchainResolver_abi, signer, TEST_URL, [signerAddress])
    ...
    describe("resolve()", () => {
        it("resolves calls to addr(bytes32)", async () => {
            ...
            expect(resultData).to.deep.equal([TEST_DB["test.eth"].addresses[ETH_COIN_TYPE]])
        })
